...
async function testFinalSimple() {
    try {
        ...
        // 1. Créer un bien pour le propriétaire (ID 5)
        ...
        // 2. Créer une demande de visiteur (sans date_visite)
        console.log('\n2. Création demande visiteur...');
        const demandeVisiteurResult = await db.query(`
            INSERT INTO demande_inscription_visiteur 
            (nom, prenoms, email, telephone, message, statut, id_bien) 
            VALUES ($1, $2, $3, $4, $5, 'en_attente', $6) 
            RETURNING id_demande, nom, email
        `, ['Jean', 'Visiteur', 'jean.visiteur@test.com', '0123456789', 'Je souhaite visiter cet appartement', id_bien]);
