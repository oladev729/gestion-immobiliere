...
export default function TenantRentals() {
  ...
  const fetchContrats = async () => {
    try {
      ...
      // Charger les charges pour chaque contrat
      const chargesMap = {};
      await Promise.all(
              ...
              <div className="col-12" key={idContrat}>
                <div className="card shadow-sm">
                  <div className="card-header d-flex justify-content-between align-items-center">
                    ...
                    <span
                      className={`badge bg-${getStatutBadge(contrat.statut_contrat)}`}
                    >
                  ...
                  <div className="card-body">
                    <div className="row g-3 mb-3">
                      ...
                      <div className="col-md-6">
                        ...
                        <p className="mb-1">
                          <span className="text-muted">Depot de garantie :</span>{' '}
                          <strong>{formatMontant(contrat.depot_garantie)}</strong>
                        </p>
                        <p className="mb-1">
                          <span className="text-muted">Debut du contrat :</span>{' '}
                          ...
                          <table className="table table-sm table-hover mb-0">
                            ...
                            <tbody>
                              {chargesContrat.map((charge, idx) => (
                                <tr key={charge.id_paiement || idx}>
                                  <td>{charge.description || 'Charge'}</td>
                                  <td className="text-danger fw-semibold">
                                    {formatMontant(charge.montant)}
                                  </td>
                                  <td>{formatDate(charge.date_paiement)}</td>
                                  <td>
                                    <span
                                      className={`badge bg-${
                                        charge.statut_paiement === 'paye'
                                          ? 'success'
